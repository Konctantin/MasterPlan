...
local computeEquivXP, computeEarliestDeparture do -- +api.GetSuggestedMissionGroups
	...
	function api.GetSuggestedMissionGroups(missions, f1, f2, f3)
		...
		for i=1,#missions do
			...
			c = c and c[1] and api.GetMissionGroupDeparture(c[1], mi) and c[1] or nil
...
function api.UpdateGroupEstimates(missions, useInactive, yield)
			...
			repeat
				...
				for i=1, mic do
					...
					if best[1] - sc < s1 then
						...
						local hi, lo = sc + s2 * ((traits[ts[mi[4]]] or 0) * 16 + na * 4 + nw), (32767-gap)*16 + traits[221]
	...
	for i=1,#missions do
		...
		if best and best[1] > 0 then
			...
			bt[4], bt.dtrait = traits[ts[mi[4]]] or 0
			local lc, cn, h
			for i=6, #mi do
				local c = mi[i]
				if c == lc then
